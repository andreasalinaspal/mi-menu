...
function Input({value,onChange,placeholder,inputRef,onKeyDown}){
  return <input ref={inputRef} type="text" value={value} onChange={onChange} placeholder={placeholder} onKeyDown={onKeyDown} style={{width:"100%",padding:"13px 16px",fontSize:16,border:`2px solid ${C.border}`,borderRadius:14,fontFamily:F.body,outline:"none",background:C.bg,boxSizing:"border-box",transition:"border-color 0.2s"}} onFocus={e=>e.target.style.borderColor=C.primary} onBlur={e=>e.target.style.borderColor=C.border}/>;
}
function Btn({children,onClick,disabled,secondary,full}){
  return <button onClick={onClick} disabled={disabled} style={{padding:"13px 20px",background:disabled?"#DDD":secondary?C.primaryLight:C.primary,color:disabled?"#999":secondary?C.primary:"white",border:"none",borderRadius:14,fontSize:15,fontWeight:600,cursor:disabled?"default":"pointer",fontFamily:F.body,transition:"all 0.2s",width:full?"100%":"auto"}}>{children}</button>;
}
...
function AddMealModal({isOpen,onClose,onSave,mealType,recipes}){
  ...
  return <Overlay onClose={onClose}><Sheet>
    <div style={{padding:"20px 24px 36px"}}>
      ...
      {step===1 && <>
        ...
        {mode==="choose" && <>
          {recipes.length===0 ? <div style={{textAlign:"center",padding:"16px 0"}}>
            ...
            {recipes.map(r=><div key={r.id} onClick={()=>setSelRecipe(r)} style={{display:"flex",gap:10,alignItems:"center",padding:"10px 12px",background:selRecipe?.id===r.id?C.primaryLight:C.surface,border:`2px solid ${selRecipe?.id===r.id?C.primary:C.border}`,borderRadius:12,cursor:"pointer",transition:"all 0.15s"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                {r.description&&<div style={{fontSize:11,color:C.textMuted,fontFamily:F.body}}>{r.description.slice(0,50)}{r.description.length>50?"...":""}</div>}
              </div>
        ...
        {canNext&&<div style={{marginTop:14}}><Btn onClick={()=>setStep(2)} full>Siguiente →</Btn></div>}
      </>}
      ...
      {step===2 && <>
        ...
        <div style={{display:"flex",gap:10,marginTop:20}}>
          <Btn onClick={()=>setStep(1)} secondary>← Atrás</Btn>
          <div style={{flex:1}}><Btn onClick={()=>{if(servings)onSave({name:mealName,servings,recipeId:selRecipe?.id||null})}} disabled={!servings} full>Guardar ✓</Btn></div>
        </div>
      </>}
    </div>
  </Sheet></Overlay>;
...
function MealDetailModal({isOpen,onClose,meal,linkedRecipe,onEditRecipe,onCreateRecipe}){
  ...
  return <Overlay onClose={onClose}><Sheet>
    ...
    <div style={{display:"flex",background:"#EDE5DD",borderRadius:12,padding:3,margin:"14px 24px 0"}}>
      ...
      {linkedRecipe ? <>
        {!editing ? <>
          ...
          <div style={{display:"flex",gap:10,marginTop:14}}>
            ...
            <button onClick={()=>{onEditRecipe(linkedRecipe.id,{ingredients:editIngredients,steps:editSteps});setEditing(false)}} style={{flex:2,padding:12,background:C.primary,color:"white",border:"none",borderRadius:12,fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:F.body}}>✓ Guardar cambios</button>
          </div>
        </>}
      </> : <>
...
/* ═══════════════════════════════════════════
   RECIPE DETAIL VIEW (inside Recetas tab)
   ═══════════════════════════════════════════ */
...
export default function MiMenu(){
  ...
  const todayKey=dk(selectedDate);
  ...
  function saveRecipe(data){
    if(recipeModal.existing){
      setRecipes(p=>p.map(r=>r.id===recipeModal.existing.id?{...r,...data}:r));
    } else {
  ...
  return <div style={{maxWidth:480,margin:"0 auto",minHeight:"100vh",background:C.bg,fontFamily:F.body,paddingBottom:70}}>
    ...
    {mainTab==="calendario"&&<>
      <div style={{background:"linear-gradient(180deg,#F0E6DC,#FAF7F4)",padding:"20px 20px 0",position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <button onClick={()=>{const p=new Date(selectedDate);p.setDate(p.getDate()-7);setSelectedDate(p)}} style={{background:"none",border:"none",fontSize:18,color:C.primary,cursor:"pointer",padding:"4px 8px",fontWeight:700}}>‹</button>
          ...
          <button onClick={()=>{const n=new Date(selectedDate);n.setDate(n.getDate()+7);setSelectedDate(n)}} style={{background:"none",border:"none",fontSize:18,color:C.primary,cursor:"pointer",padding:"4px 8px",fontWeight:700}}>›</button>
        </div>

        {/* Week strip */}
        <div style={{display:"flex",gap:4,marginBottom:12}}>
          {weekDates.map((d,i)=>{const isSel=dk(d)===dk(selectedDate);const isToday=dk(d)===dk(today);
      ...
      {calTab==="hoy"&&<div style={{padding:"0 20px 20px",animation:"fadeIn 0.2s ease"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {MEAL_TYPES.map(mt=>{const meal=todayMeals[mt.key];
            return <div key={mt.key}>{meal ?
              <div onClick={()=>setDetailModal({open:true,meal})} style={{background:"white",borderRadius:16,padding:14,borderLeft:`4px solid ${dayColor.border}`,cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.06)",display:"flex",gap:12,alignItems:"center",transition:"transform 0.15s"}}
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11,color:C.textMuted,fontFamily:F.body,marginTop:2}}>👥 {meal.servings} pers.{meal.recipeId?" · 📋 Receta":""}</div>
                </div>
                <div style={{color:dayColor.border,fontSize:20,flexShrink:0}}>›</div>
              </div>
            : <div onClick={()=>setAddModal({open:true,mealType:mt})} style={{background:"white",borderRadius:16,padding:"16px 14px",border:"2px dashed #E0D6CE",cursor:"pointer",display:"flex",gap:12,alignItems:"center",transition:"border-color 0.2s"}}
      ...
      {calTab==="semana"&&<div style={{padding:"0 20px 20px",animation:"fadeIn 0.2s ease"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {weekDates.map((d,i)=>{const k=dk(d);const dm=meals[k]||{};const dc=DAY_COLORS[d.getDay()];
    ...
    {mainTab==="recetas"&&<>
      ...
      {viewRecipe ? <RecipeDetail recipe={viewRecipe} onBack={()=>setViewRecipe(null)} onEdit={r=>{setRecipeModal({open:true,existing:r});setViewRecipe(null)}} onDelete={id=>{deleteRecipe(id)}}/>
      : <div style={{padding:"0 20px 120px"}}>
        ...
        {recipes.length===0 ? <div style={{textAlign:"center",padding:"40px 0"}}>
          ...
          <p style={{fontFamily:F.body,fontSize:13,color:C.textFaint,margin:"6px 0 0"}}>Creá tu primera receta para empezar</p>
    ...
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:50,background:"white",borderTop:"1px solid #E8DDD6",display:"flex",maxWidth:480,margin:"0 auto"}}>
      {[{k:"calendario",icon:"📅",l:"Calendario"},{k:"recetas",icon:"📖",l:"Recetas"}].map(t=>
        <button key={t.k} onClick={()=>{setMainTab(t.k);if(t.k==="recetas")setViewRecipe(null)}} style={{flex:1,padding:"10px 0 8px",border:"none",background:"transparent",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,transition:"all 0.2s"}}>
